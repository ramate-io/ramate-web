/*
 * Email Activity (beta)
 *
 * The Beta endpoints for the new Email Activity APIs - functionality is subject to change without notice. You may not have access to this Beta endpoint.  Email Activity offers filtering and search by event type for two days worth of data. There is an optional add-on to store 60 days worth of data. This add-on also gives you access to the ability to download a CSV of the 60 days worth of email event data. The Beta endpoints for the new Email Activity APIs - functionality is subject to change without notice. You may not have access to this Beta endpoint.  Email Activity offers filtering and search by event type for two days worth of data. There is an optional add-on to store 60 days worth of data. This add-on also gives you access to the ability to download a CSV of the 60 days worth of email event data.
 *
 * The version of the OpenAPI document: 
 * 
 * Generated by: https://openapi-generator.tech
 */

/// GetMcContactsCount200ResponseBillableBreakdown : `billable_breakdown` will only appear to the parent user in an account with subusers.



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct GetMcContactsCount200ResponseBillableBreakdown {
    /// The sum of all the subuser's billable contacts
    #[serde(rename = "total", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub total: Option<Option<serde_json::Value>>,
    /// A map of each subuser's billable contact usage. Each key is the subuser's ID and each value is the usage thus far this month.
    #[serde(rename = "breakdown", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub breakdown: Option<Option<serde_json::Value>>,
}

impl GetMcContactsCount200ResponseBillableBreakdown {
    /// `billable_breakdown` will only appear to the parent user in an account with subusers.
    pub fn new() -> GetMcContactsCount200ResponseBillableBreakdown {
        GetMcContactsCount200ResponseBillableBreakdown {
            total: None,
            breakdown: None,
        }
    }
}


